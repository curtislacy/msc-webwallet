...
from btc_utils_parsing import *
from btc_utils_obelisk import *
...
def parse():
    ...
    btc_globals.d=options.debug_mode
    ...
    if requested_block_height == None:
        ...
        btc_globals.last_block=starting_block_height

    else:
        starting_block_height=requested_block_height

    ending_block_height = get_last_height()

    info('starting parsing at block '+str(starting_block_height) + ' and ending at ' + str( ending_block_height ))

    for block_num in range( int( starting_block_height ), int( ending_block_height )):
        raw_block, err = run_command( "sx fetch-block-header " + str( block_num ))
